...
public class PokerSquares {
	 ...
	 */
	public int play(boolean verbose, long seed) {
	...
	public int play() {
		try {
			...
			/**
			deck.clear();
			deck.push(Card.getCard("4D"));
			deck.push(Card.getCard("7H"));
			deck.push(Card.getCard("3H"));
			deck.push(Card.getCard("9H"));
			deck.push(Card.getCard("5S"));
			deck.push(Card.getCard("TD"));
			deck.push(Card.getCard("KS"));
			deck.push(Card.getCard("2C"));
			deck.push(Card.getCard("6D"));
			deck.push(Card.getCard("5D"));
			deck.push(Card.getCard("TH"));
			deck.push(Card.getCard("JH"));
			deck.push(Card.getCard("7S"));
			deck.push(Card.getCard("TS"));
			deck.push(Card.getCard("4H"));
			deck.push(Card.getCard("TC"));
			deck.push(Card.getCard("AH"));
			deck.push(Card.getCard("8C"));
			deck.push(Card.getCard("8D"));
			deck.push(Card.getCard("8H"));
			deck.push(Card.getCard("3C"));
			deck.push(Card.getCard("8S"));
			deck.push(Card.getCard("5H"));
			deck.push(Card.getCard("4C"));
			deck.push(Card.getCard("6H"));
			//*/
			// clear grid
...
//		new PokerSquares(new RandomPlayer(), system).playSequence(3, 0, true);
		...
		//for (int i=0; i<16; i++) {
            System.out.println("Tournament evaluation demo:");

        	ArrayList<PokerSquaresPlayer> players = new ArrayList<PokerSquaresPlayer>();
            players.add(new GreedyMCPlayer(8));
            players.add(new Reath_GreedyProbabilityMCPlayer(null));
            players.add(new Reath_GreedyProbabilityMCPlayer(8));
            PokerSquaresPointSystem system = PokerSquaresPointSystem.getBritishPointSystem();
            ...
            for (int i=0; i<30; i++) {
            	int max = 0;
            	int maxI = 0;
            	seed = 1575070021291l; //System.currentTimeMillis();
            	for (int j=0; j<players.size(); j++) {
					int score = new PokerSquares(players.get(j), system).play(false, seed);
            	...
            	System.out.println("Winner: " + players.get(maxI).getName() + " averages: " +  ((double)totals[0]/(double)(i+1)) + " " + ((double)totals[1]/(double)(i+1)) + " " +((double)totals[2]/(double)(i+1)) + " Seed: " + seed);
            }

            players.add(new Reath_GreedyProbabilityMCPlayer(8));
            players.add(new Reath_GreedyProbabilityMCPlayer(9));
            players.add(new Reath_GreedyProbabilityMCPlayer(10));
            players.add(new Reath_GreedyProbabilityMCPlayer(11));
            players.add(new Reath_GreedyProbabilityMCPlayer(13));
            players.add(new Reath_GreedyProbabilityMCPlayer(14));
            players.add(new Reath_GreedyProbabilityMCPlayer(15));
            ...
            PokerSquares.playTournament(players, systems, 1, seed);  // play 10 games for each player under each scoring system
        //}
	}
}
